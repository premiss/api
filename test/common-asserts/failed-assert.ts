...
export const failureAssertion = ((examResult: ExamResult, proofStep: ProofStep): void =>
{
	...
	assert.ok(examResult.executionError, "The error is not defined");
	assert.equal((examResult.executionError?.error as Error), error, `The error should have been the ${error.message} error`);
	assert.equal(examResult.executionError?.proofStep, proofStep, `The error should be for step ${proofStep} but value was ${examResult.executionError?.proofStep || "undefined"}`);
}) as FailureAssertion;
